...
async function loadFieldDefs(showId: string): Promise<FieldDef[]> {
  const [showRow] = await db
    .select()
    .from((await import('@/db/schema')).shows)
    .where(eq((await import('@/db/schema')).shows.id, showId))
    .limit(1);
...
function formatRecentLeads(leads: RecentLeadSummary[]): string {
  if (leads.length === 0) return '(none yet — this is the first lead at this show)';
  return leads
    .map((l) => {
...
You are talking to ${repName ?? 'a sales rep'}, a trade-show booth rep at "${showName}". They are DEBRIEFING you about a lead they just met — the lead is NOT here, NOT listening, NOT part of this conversation. You are essentially the rep's voice-driven notebook.
...
CURRENT KNOWN INFO about this lead${opportunityCode ? ` (opportunity ${opportunityCode})` : ' (new — opportunity will be assigned later if it matches an existing lead)'}:
...
${fields.map((f) => `  ${f.required ? '★' : ' '} ${f.key}: ${f.label}${f.aiHint ? ` — ${f.aiHint}` : ''}`).join('\n')}`;
}
